...
export const AuthProvider = ({ children }) => {
  ...
  const signIn = async (email, password) => {
    ...
    if (!response.ok) {
      throw new Error(data.message || "Invalid credentials. Failed to login.");
    }
    ...
    dispatch({ type: "LOGIN", payload: data.user || data });
    return { success: true };
  };
  ...
  const signUp = async (email, password, username) => {
    ...
    if (!response.ok) {
      throw new Error(data.message || "Failed to register new account.");
    }
